...
export default class OdlAccountRecord extends LightningElement {
    ...
    get hasAccount() { return this.account != null && this.account.Id != null; }
    ...
    // Files and photos come from the account's real attached documents — never hardcoded tiles.
    @wire(getRelatedListRecords, {
        parentRecordId: '$recordId',
        relatedListId: 'AttachedContentDocuments',
        fields: ['ContentDocument.Id', 'ContentDocument.Title', 'ContentDocument.FileType']
    })
    wiredFiles({ error, data }) {
        if (data) {
            this.files = (data.records || []).map(r => ({
                id: r.id,
                title: (r.fields.Title && r.fields.Title.value) || (r.fields.FileType && r.fields.FileType.value) || 'File'
            }));
        } else if (error) {
            this.files = [];
        }
